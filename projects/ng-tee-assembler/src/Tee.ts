...
        <div class='front_foot' body-part></div>`;
      ...
      // *** MODIFIED PART ***
      // Set size from config if provided
      if(this.config.size) {
...
        
        // *** MODIFIED PART ***
        // Get colors from config object instead of data-attributes
...
          await this.getTeeImage(bodycolor as string, feetcolor as string, this.config.coloringMode as string);
        } else {
...
  private originCoord : any;
  private config: TeeConfig; // *** ADDED ***
...
  unbindContainer() {
    if (this.container) {
      // Clean up innerHTML and remove ID
      this.container.innerHTML = '';
...
  setTeeEyesVariables() {
    if (this.teeEyesVariables || !this.container) return; // Prevent re-querying
...
    this.moveTeeEyesFunction = (e: MouseEvent) => {
      if (!this.line) return; // Guard against unmounted element
      this.originCoord = {
